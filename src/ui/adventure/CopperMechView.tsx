...
import { runMods, applyRelicsToState, applyDeckBuffs } from '@adventure/relics';
...
export function CopperMechView({ run, fightSeed, onDetail }: {
  ...
  const { registry, initial } = useMemo(() => {
    ...
    let initial = buildCopperMechState(registry, playerDeck(run.leaderId, buffedDeck), fightSeed, run.maxHp);
  ...
  return (
    <RegistryProvider value={registry}>
      <div className="advcombat__bar advcombat__bar--copper">
        ...
        {/* State the rule. Seeing a wounded run HP on the map and a full bar in here
            otherwise reads as a bug rather than as the level playing field it is. */}
        <span className="advhud__stat" title="Every attempt starts from your leader's full HP, whatever the run's state — so a score means the same thing every time, and HP relics cannot buy a better one.">
          ❤ Full HP · run HP and HP relics do not apply
        </span>
