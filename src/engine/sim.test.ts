...
describe('sim', () => {
  it('plays an AI-vs-AI game to a decisive result', () => {
    const r = simulateGame(registry, [deckAggro, deckControl], 1);
  ...
  it('is deterministic for a fixed seed', () => {
    const a = simulateGame(registry, [deckAggro, deckControl], 7);
    const b = simulateGame(registry, [deckAggro, deckControl], 7);
    expect(b).toEqual(a);
  });

  it('aggregates a batch with win counts summing to the game count', () => {
    const res = runBatch(registry, [deckAggro, deckControl], 12);
  ...
  it('runs a deck against a small field with matchups + aggregated cards', () => {
    const res = runField(registry, deckAggro, [{ deck: deckControl, name: 'Control' }], 6);
  ...
  it('round-robins a meta matrix with symmetric, complementary cells', () => {
    ...
    const res = runMeta(registry, field, 6);
