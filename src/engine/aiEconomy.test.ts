...
describe('AI understands the new economy', () => {
  ...
  it('can cast a pipped card off the bank alone, with no generic energy at all', () => {
    // Ash Cloud is 0 energy + 1 fire. A player on 0 energy but holding 1 banked fire must be
    ...
    // damage has none, so Firebolt is priced in energy. Hypnotic Patterns replaced it and in
    // turn stopped qualifying when the curve-compression retune lifted the cheap end of the
    // pool off zero — it is now 1 energy + 1 water, so it no longer tests a bank-only cast.)
    const s = blankState({ round: 3 });
    withHand(s, 0, ['ash-cloud']);
    ...
    const def = starterRegistry.cards.get('ash-cloud')!;
    expect(canAfford(s.players[0], def.cost).ok).toBe(true);
