...
describe('aiTuning harness fairness', () => {
  it('scores identical weights as exactly even, on every seed set', () => {
    ...
    // 40 min, not 15: this full-planning match was already marginal against its old timeout
    // (the whole suite runs ~3 h) and tipped over it. NOT because games got longer — the
    // retaliation rework measured 37.5 -> 35.8 avg turns, slightly shorter. Raised rather
    // than skipped: the symmetry it checks is a real correctness property of the scoreboard.
  }, 2_400_000);
